...
def make_mean_psth_scenes(meandfs,psth_cols,fig_title,fig_file_path):
        ...
        p = sns.FacetGrid(meandfs, col="stim_cond", col_wrap=psth_cols, size=5)
...
opts.animalid = 'CE077'
opts.session = '20180523'
opts.acquisition = 'FOV1_zoom1x'
...
for type_idx,t_type in enumerate(trace_type):
    ...
    for ridx in cell_rois:
        ...
        for cfg_count,cfg_key in enumerate(data_grp[curr_slice].keys()):
            stim_cond = np.array(data_grp['/'.join([curr_slice,cfg_key,'grating_cond'])])[0]*45
