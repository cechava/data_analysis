...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug  3 17:55:13 2019
...
opts.response_type = 'norm_df'
opts.filter_crit = 'zscore'
opts.filter_thresh = 1
...
resp_diff = np.ones((40,filt_response_array.shape[1]))*np.nan
mod_idx = np.ones((40,filt_response_array.shape[1]))*np.nan


for img_count, cond_idx in enumerate(range(0,nconfigs,3)):
    if filter_matrix[cond_idx,ridx]:
        #original>medium complex
        resp_diff[2*img_count,:] = filt_response_array[cond_idx,:] - filt_response_array[cond_idx+2,:]
        mod_idx[2*img_count,:] = np.true_divide(filt_response_array[cond_idx,:] - filt_response_array[cond_idx+2,:],
                                           filt_response_array[cond_idx,:] + filt_response_array[cond_idx+2,:])       
       
       #medium>low complex
        resp_diff[(2*img_count)+1,:] = filt_response_array[cond_idx+2,:] - filt_response_array[cond_idx+1,:]
        mod_idx[(2*img_count)+1,:] = np.true_divide(filt_response_array[cond_idx+2,:] - filt_response_array[cond_idx+1,:],
                                           filt_response_array[cond_idx+2,:] + filt_response_array[cond_idx+1,:]) 
...
plt.bar(bar_loc[1:len(bar_loc):3],img_response_mean[2:len(bar_loc):3],width,color = 'r',yerr = img_response_se[2:len(bar_loc):3])
plt.bar(bar_loc[2:len(bar_loc):3],img_response_mean[1:len(bar_loc):3],width,color = 'g',yerr = img_response_se[1:len(bar_loc):3])
...
filter_response_cond_per_neuron = filter_response_cond_per_neuron[[0,2,1],:]
...
plt.bar(bar_loc[0],filter_response_cond_mean_neuron[0],width,color = 'b',yerr = filter_response_cond_se_neuron[0])
plt.bar(bar_loc[1],filter_response_cond_mean_neuron[1],width,color = 'r',yerr = filter_response_cond_se_neuron[1])
plt.bar(bar_loc[2],filter_response_cond_mean_neuron[2],width,color = 'g',yerr = filter_response_cond_se_neuron[2])
...
mdif_se_dset = data_grp.create_dataset('/'.join([curr_slice, 'diff_se_across_neurons' ]), sem_diff.shape, sem_diff.dtype)
mdif_se_dset[...] = sem_diff
